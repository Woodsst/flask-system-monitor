...
def test_client_log_time_work(api_client):
    api_client.post(
        path="/client",
        json={"username": user, "pass": password},
        headers=header,
    )
...
def test_client_log_time_work_error(api_client):
    api_client.post(
        path="/client", json={"username": "other_client", "pass": password}
    )
    client_id = base64.b64encode(f"other_client:{password}".encode()).decode()
    response = api_client.post(
        "/client/other_client/time", headers={"Authorization": client_id}
    )
    assert response.status_code == 406
    assert response.json() == {"error": "log is empty"}
    response = api_client.post(
        "/client/other_client/time",
