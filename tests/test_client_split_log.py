...
def test_split_log(api_client):
    ...
    response = api_client.post(
        path=f"/client/{user}/time/report?start={1646650624}&end={1646650626}",
        headers=header,
    )
...
def test_split_log_empty_start_end(api_client):
    response = api_client.post(
        path=f"/client/{user}/time/report?start=&end=", headers=header
    )
    response_json = response.json()
    assert response.status_code == 200
    assert isinstance(response_json["payload"], list)
...
def test_split_log_for_errors(api_client):
    response = api_client.post(
        path=f"/client/{user}/time/report"
        "?start=badargument&end=badargumenttwo",
        headers=header,
    )
    response_json = response.json()
    ...
    response = api_client.post(
        "/client/other_client/time/report?start=&end=",
        headers={"Authorization": "bad client id"},
    )
