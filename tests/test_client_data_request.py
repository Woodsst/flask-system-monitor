...
def test_client_data_request_202(api_client, psql):
    ...
    response = api_client.post(f'/client/{user}', data=data,
                                 headers=header)
...
def test_client_data_request_401(api_client):
    api_client.post(path='/client', json={'username': user, 'pass': password})
    response = api_client.post(f'/client/{user}', data='',
                                 headers=header)
    assert response.status_code == 400
    response = api_client.post(f'/client/{user}', headers={'Authorization': f'Basic afgjn123'}, data=data)
